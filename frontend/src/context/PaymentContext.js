...
export function PaymentProvider({ children }) {
  ...
  const [status, setStatus] = useState({ is_paid: false, paid_until: null, plan: null });
  ...
  const refresh = useCallback(async (paymentIntentId) => {
    ...
    try {
      ...
      if (e?.code !== 'TokenExpired') {
        ...
        setStatus({ is_paid: false, paid_until: null, plan: null });
      }
      return null;
    } finally {
      setChecking(false);
  ...
  return (
    <PaymentCtx.Provider value={{ isPaid: status.is_paid, paidUntil: status.paid_until, plan: status.plan, checking, error, refresh }}>
      {children}
    </PaymentCtx.Provider>
  );
}
