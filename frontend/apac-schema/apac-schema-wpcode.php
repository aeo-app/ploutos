/**
 * APAC Relocation - Structured Data (server-rendered JSON-LD)
 *
 * WPCode setup: Code Type = PHP Snippet, Insertion = Auto Insert, Location = Run Everywhere.
 * Prints MovingCompany schema on the Home page + 5 service pages and FAQPage schema
 * on 3 blog posts, directly into the page HTML <head>. All other pages get nothing.
 * Validated: 0 errors against schema.org v29.1 + Google requirements.
 */
if ( ! function_exists( 'apac_output_schema' ) ) {

    function apac_schema_data() {
        $json = <<<'APAC_SCHEMA_JSON'
{
  "/": {
...
APAC_SCHEMA_JSON;
    ...
    function apac_output_schema() {
        if ( is_admin() || is_feed() || is_404() ) {
            return;
        }
        $path = wp_parse_url( $_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH );
        $path = strtolower( trailingslashit( $path ? $path : '/' ) );
        if ( is_front_page() ) {
            $path = '/';
        }
        $all = apac_schema_data();
        if ( ! is_array( $all ) || empty( $all[ $path ] ) ) {
            return;
        }
        $json = wp_json_encode(
            $all[ $path ],
        ...
        if ( $json ) {
            echo "\n<!-- APAC Relocation schema -->\n<script type=\"application/ld+json\" class=\"apac-schema\">\n" . $json . "\n</script>\n";
        }
    }

    add_action( 'wp_head', 'apac_output_schema', 5 );
}
